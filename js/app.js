...
function getActiveGemstones(){
  try{ const raw=localStorage.getItem('rosairoGemstones'); if(raw){ const parsed=JSON.parse(raw); if(Array.isArray(parsed)) return parsed; } }catch(e){}
  return typeof GEMSTONES!=='undefined'?GEMSTONES:[];
}
function visibleGemstones(){ return getActiveGemstones().filter(p=>p.status!=='hidden'); }
function waLink(p){
...
function card(p){
  ...
  return `<article class="stone-card ${p.status==='sold'?'sold-card':''}"><a class="stone-image" href="stone.html?id=${p.id}"><img src="${img}" alt="${p.name}"><span>${p.status==='sold'?'SOLD OUT':p.subcategory}</span></a><div class="stone-info"><div class="stone-meta">${p.carat} · ${p.origin}</div><h3>${p.name}</h3><p>${p.status==='sold'?'Sold Out':p.treatment}</p><a class="more-link" href="stone.html?id=${p.id}">More Details <b>→</b></a></div></article>`;
}
function renderCategoryPage(){
 ...
 const grid=document.getElementById('stoneGrid'); let list=visibleGemstones().filter(p=>p.category===id); const search=document.getElementById('stoneSearch');
 function draw(){
...
function renderStone(){
 ...
 const id=new URLSearchParams(location.search).get('id'); const p=getActiveGemstones().find(x=>x.id===id && x.status!=='hidden');
